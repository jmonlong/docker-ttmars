...
ref = randSeq(2*10e5)
# write it to ref.fa
recs = [SeqRecord(ref, id=chr_prefix + '1', description='')]
...
out_vcf.write('##contig=<ID={}1,length={}>\n'.format(chr_prefix, len(ref)))
...
out_vcf.write(chr_prefix + '1\t{pos}\t.\t{ref}\t{alt}\t100\t.\tSVTYPE=INS;SVLEN={svl}\tGT\t0/1\n'.format(pos=ins_pos+1, ref=ref_s, alt=alt_s, svl=len(ins_seq)))
# one 500bp hom deletion at ~80% of the chromosome
del_start = int(len(ref)*.8)
del_end = del_start + 501
ref_s = ref[del_start:del_end]
alt_s = ref_s[0]
out_vcf.write(chr_prefix + '1\t{pos}\t.\t{ref}\t{alt}\t100\t.\tSVTYPE=DEL\tGT\t1/1\n'.format(pos=del_start+1, ref=ref_s, alt=alt_s))
out_vcf.close()

# make two assembled haplotypes and let's say the last 90-95%
# of the reference is not covered
...
hap1 = hap1[:ins_pos] + ins_seq_1 + hap1[ins_pos:(ins_pos+5)] + \
    ins_seq_2 + hap1[(ins_pos+5):(ins_pos+15)] + \
